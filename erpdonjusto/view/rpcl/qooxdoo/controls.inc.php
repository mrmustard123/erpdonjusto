...
class QControl extends FocusControl
{
    ...
    function fireEvent($event, $phpevent, $serverevent)
    {
      if ($phpevent!=null)
      {
        if ($serverevent==$phpevent)
        {
          $this->callEvent($event,array());
        }
      }
    }
    ...
    function init()
    {
      ...
      {
        $this->fireEvent('onclick',$this->_onclick, $serverevent->asString());
...
<?php
    ...
    function dumpHeaderCode()
    {
      ...
      if (!defined('buildGUI'))
      {
        ...
        echo self::$guicode."\n";
        echo '}'."\n";
        echo "function qx_processevent(event,phpevent)\n";
        echo "{\n";
