<?php
<object class="view_hist_posicion" name="view_hist_posicion" baseclass="Page">
  <property name="Background"></property>
  <property name="Caption">POSICION HISTORIA</property>
  <property name="DocType">dtNone</property>
  <property name="Encoding">Unicode (UTF-8)            |utf-8</property>
  <property name="Height">418</property>
  <property name="IsMaster">0</property>
  <property name="Name">view_hist_posicion</property>
  <property name="Width">251</property>
  <property name="OnShow">view_hist_posicionShow</property>
  <property name="OnShowHeader">view_hist_posicionShowHeader</property>
  <object class="Edit" name="apiario1" >
    <property name="DataField">apiario</property>
    <property name="Datasource">dshistorico_posicion1</property>
    <property name="Height">21</property>
    <property name="Name">apiario1</property>
    <property name="Top">268</property>
    <property name="Width">121</property>
  </object>
  <object class="Edit" name="posicion_id1" >
    <property name="DataField">posicion_id</property>
    <property name="Datasource">dshistorico_posicion1</property>
    <property name="Height">21</property>
    <property name="Name">posicion_id1</property>
    <property name="Top">216</property>
    <property name="Width">121</property>
  </object>
  <object class="Memo" name="comentario1" >
    <property name="DataField">comentario</property>
    <property name="Datasource">dshistorico_posicion1</property>
    <property name="Height">89</property>
    <property name="Lines">a:0:{}</property>
    <property name="Name">comentario1</property>
    <property name="Top">94</property>
    <property name="Width">185</property>
  </object>
  <object class="Label" name="Label1" >
    <property name="Caption">Comentarios</property>
    <property name="Height">13</property>
    <property name="Name">Label1</property>
    <property name="Top">72</property>
    <property name="Width">75</property>
  </object>
  <object class="Label" name="Label2" >
    <property name="Caption">Posicion:</property>
    <property name="Height">13</property>
    <property name="Name">Label2</property>
    <property name="Top">200</property>
    <property name="Width">75</property>
  </object>
  <object class="Label" name="Label3" >
    <property name="Caption">Apiario:</property>
    <property name="Height">13</property>
    <property name="Name">Label3</property>
    <property name="Top">253</property>
    <property name="Width">75</property>
  </object>
  <object class="DateTimePicker" name="DateTimePicker1" >
    <property name="Caption">DateTimePicker1</property>
    <property name="Height">17</property>
    <property name="Name">DateTimePicker1</property>
    <property name="Top">47</property>
    <property name="Width">200</property>
  </object>
  <object class="Label" name="Label4" >
    <property name="Caption">Fecha</property>
    <property name="Height">13</property>
    <property name="Name">Label4</property>
    <property name="Top">24</property>
    <property name="Width">75</property>
  </object>
  <object class="Button" name="Button1" >
    <property name="Caption">Gravar</property>
    <property name="Height">25</property>
    <property name="Name">Button1</property>
    <property name="Top">312</property>
    <property name="Width">75</property>
    <property name="OnClick">Button1Click</property>
  </object>
  <object class="Database" name="dbamenoec1_erpdonjusto1" >
        <property name="Left">160</property>
        <property name="Top">212</property>
    <property name="Connected"></property>
    <property name="DatabaseName">apicolad_erpdonjusto</property>
    <property name="Host">localhost</property>
    <property name="Name">dbamenoec1_erpdonjusto1</property>
    <property name="UserName">apicolad_justo</property>
    <property name="UserPassword">aA1NfDBW5Wlm</property>
  </object>
  <object class="Table" name="tbhistorico_posicion1" >
        <property name="Left">160</property>
        <property name="Top">262</property>
    <property name="Active">1</property>
    <property name="Database">database_module01.dbapicolad_erpdonjusto1</property>
    <property name="MasterFields">a:0:{}</property>
    <property name="MasterSource"></property>
    <property name="Name">tbhistorico_posicion1</property>
    <property name="TableName">historico_posicion</property>
  </object>
  <object class="Datasource" name="dshistorico_posicion1" >
        <property name="Left">160</property>
        <property name="Top">312</property>
    <property name="Dataset">tbhistorico_posicion1</property>
    <property name="Name">dshistorico_posicion1</property>
  </object>
</object>
?>
